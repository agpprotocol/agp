"""Any-of signers trust primitive."""
...
class AnyOfSignersPrimitive(Primitive):
    """Require at least one signer from a configured identity set."""
    ...
    def validate(
        ...
        if unknown:
            raise ValueError(
                f"any_of_signers unknown members: {unknown}"
            )

        if missing:
            raise ValueError(
                f"any_of_signers missing members: {missing}"
            )
        ...
        if (
            ...
            raise ValueError(
                "any_of_signers.signer_ids must contain "
        ...
        if any(
            ...
            raise ValueError(
                "any_of_signers.signer_ids entries "
        ...
        if signer_ids != sorted(signer_ids):
            raise ValueError(
                "any_of_signers.signer_ids must be "
    ...
    def evaluate(
        ...
        return PrimitiveResult.unsatisfied_result(
            ...
            observed=observed,
            expected=expected,
            failure_code="ANY_OF_SIGNERS_MISSING",
        )
