"""Exactly-one-of signers trust primitive."""
...
class ExactlyOneOfSignersPrimitive(Primitive):
    """Require exactly one signer from a configured identity set."""
    ...
    def validate(
        ...
        if unknown:
            raise ValueError(
                f"exactly_one_of_signers unknown members: {unknown}"
            )

        if missing:
            raise ValueError(
                f"exactly_one_of_signers missing members: {missing}"
            )
        ...
        if (
            ...
            raise ValueError(
                "exactly_one_of_signers.signer_ids must contain "
        ...
        if any(
            ...
            raise ValueError(
                "exactly_one_of_signers.signer_ids entries "
        ...
        if signer_ids != sorted(signer_ids):
            raise ValueError(
                "exactly_one_of_signers.signer_ids must be "
    ...
    def evaluate(
        ...
        return PrimitiveResult.unsatisfied_result(
            ...
            observed=observed,
            expected=expected,
            failure_code="EXACTLY_ONE_OF_SIGNERS_NOT_SATISFIED",
        )
