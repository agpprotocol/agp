#!/usr/bin/env python3
"""Reproduce signed TPE 2.6 results from wheels outside the repository."""
...
EXTERNAL = (
    ROOT
    / "trust_primitive_engine/examples/external-package-tpe26"
)
...
def main() -> int:
    with tempfile.TemporaryDirectory(
        prefix="agp-tpe26-external-reproduction-"
    ) as raw:
        temp = Path(raw)
        dist = temp / "dist"
        env_dir = temp / "venv"
        ...
        agp_wheels = sorted(dist.glob("agp_tpe-2.6.0-*.whl"))
        example_wheels = sorted(
            dist.glob("tpe26_external_reproduction-1.0.0-*.whl")
        )
        ...
        completed = run(
            [str(python), "-m", "tpe26_external_reproduction"],
            cwd=run_dir,
            env=clean_env,
            capture=True,
        )
        ...
        assert "TPE_2_6_EXTERNAL_REPRODUCTION_PASS=2/2" in output, output
        assert "CASE=satisfied" in output, output
        assert "CASE=issuer-not-allowed" in output, output
        assert output.count("RESULT_SHA256=") == 2, output
        ...
        print(output, end="")
        print("TPE 2.6 external reproduction: 2/2 passed")
