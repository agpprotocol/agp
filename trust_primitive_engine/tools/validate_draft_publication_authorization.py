#!/usr/bin/env python3
"""Validate explicit authorization to publish one exact draft Release."""
...
def read_exact_draft(
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("draft Release response is not an object")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate authorization for a verified draft Release "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"AUTHORIZED_DRAFT_RELEASE_TAG="
        f"{candidate['release_tag']}"
    )
    print(f"DRAFT_RELEASE_SHA256={draft_digest}")
    print("PUBLICATION_AUTHORIZATION_VALID=yes")
    print("RELEASE_PUBLISHED=no")
    print("PYPI_PUBLISHED=no")
    print("AGP_DRAFT_PUBLICATION_AUTHORIZATION_PASS")
    return 0
