...
A shared 12-vector matrix compares the complete Python and Go evaluation
objects for bounded `all_of`, `any_of`, and `not` trees with TPE 2.6 leaves.

It covers nested truth semantics, complete-tree evaluation without
short-circuiting, recursive `children`, deterministic failure projection,
suppression beneath satisfied `any_of` and `not`, and top-level ordering.

```bash
python trust_primitive_engine/tools/test_tpe26_go_composition_evaluation.py
```

Expected marker:

```text
TPE 2.6 Python/Go composition evaluation parity: 12/12 passed
```

Policy-reference behavior remains covered by the existing frozen-profile
reproduction; this matrix isolates composition semantics without references.
...
A shared 27-vector matrix verifies that Python and Go accept and reject the
same requirement forms for the three TPE 2.6 evidence-provenance predicates.

The matrix covers valid minimal forms, missing and unknown members, invalid
identifiers, empty and oversized sets, duplicate and non-canonical sets,
invalid evidence types, wrong JSON types, and the complete `minimum` boundary.

```bash
python trust_primitive_engine/tools/test_tpe26_go_validation.py
```

Expected marker:

```text
TPE 2.6 Python/Go requirement validation parity: 27/27 passed
```

The matrix compares runtime acceptance semantics. It does not claim that both
implementations expose identical human-readable error messages.
